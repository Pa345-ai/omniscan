...
async function main() {
  ...
  console.log('\nNow check MoonPay:');
  console.log(`  mp --json token search --query "${mint.publicKey.toString()}" --chain solana | jq '.items[0].symbol, .items[0].name'`);
  console.log(`  mp --json token retrieve --token "${mint.publicKey.toString()}" --chain solana | jq '.symbol, .name'`);
